...
3. **Shallow history in the root corpus.** The root corpus (Jaffle Shop) has 92 commits over five years, which is thin for co-change and fragility evidence; any such figure taken from it is illustrative, not statistical. This limitation is scoped to that corpus. The nested corpus (Transfermarkt) is not shallow — roughly seven years of history, first commit 2019-08-04 — so it is *not* the reason Tally asserts no co-change there. The producer limitation below is, and it applies to both corpora.
4. **No co-change evidence from the producer.** The workspace.json producer withholds behavioral values by design, and emits no commit-graph evidence: `generated.coChange` is specified by the spec and left unemitted by `@workspacejson/cli@0.5.0`, the producer used here. The join exercises key membership, not value reading. This holds regardless of how deep a corpus's history is.
